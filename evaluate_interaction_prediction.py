...
handler = logging.FileHandler(filename='./log/TEST-'+datetime.strftime(datetime.now(), '%Y-%m-%d@%H-%M-%S')+'.log')
...
output_fname = "results/interaction_prediction_jodie.txt"
...
with trange(train_end_idx, test_end_idx) as progress_bar:
    for j in progress_bar:
        ...
        if userid in predicted_user:
            continue
        predicted_user.append(userid)
        ...
        closest = torch.min(euclidean_distances).data.cpu().numpy()
        ...
        logger.info('User=%d, Item=%d, Ranking=%d, Distance=%f, Closest=%f' % (userid, itemid, true_item_rank, true_item_distance, closest))
...
logger.info('Testing complete and saving results now')
# CALCULATE THE PERFORMANCE METRICS
performance_dict = dict()
ranks = validation_ranks
mrr = np.mean([1.0 / r for r in ranks])
rec10 = sum(np.array(ranks) <= 50)*1.0 / len(ranks)
...
metrics = ['Mean Reciprocal Rank', 'Recall@50']
...
logger.info('Results file path: %s' % output_fname)
fw.flush()
fw.close()
